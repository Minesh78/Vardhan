import React from 'react'

const Contact = () => {
  return (
    <>
       <div className='bg-[#904B55] h-14 relative'> </div>
      
      <div className=' mt-20 md:mt-32 md:w-[60%] w-[80%] mx-auto text-center flex flex-col gap-10 text-2xl' >

        <h1><span className='font-bold'>For internship and job opportunities:</span> <br/>Email id - jobs.vvardhan@gmail.com</h1>

        <h1>
        <span className='font-bold'>Principal architect, Vardhan Vadgave :</span>
         <br/>vvardhan@gmail.com

        </h1>

        <h1> <span className='font-bold'>Contact:</span>

         <br/>+91 9175975097</h1>
       </div>
      
    </>
  )
}

export default Contact